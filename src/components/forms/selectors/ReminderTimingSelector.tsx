  ...
  const getColorClasses = (color: string, isSelected: boolean) => {
    const colorMap: Record<string, { bg: string; border: string; icon: string; text: string }> = {
      red: {
        ...
        icon: isSelected ? 'bg-red-500/20 text-red-400' : 'bg-white/10 text-gray-200',
        text: 'text-red-300'
      },
      orange: {
        bg: isSelected ? 'from-orange-500/20 to-amber-500/20' : 'bg-white/5',
        border: isSelected ? 'border-orange-500/50' : 'border-white/10',
        icon: isSelected ? 'bg-orange-500/20 text-orange-400' : 'bg-white/10 text-gray-200',
      ...
      indigo: {
        ...
        icon: isSelected ? 'bg-indigo-500/20 text-indigo-400' : 'bg-white/10 text-gray-200',
            ...
            >
              {option.popular && (
                <div className="absolute -top-2 -right-2">
                  <span className="inline-flex items-center px-2 py-0.5 text-xs sm:text-sm sm:text-base sm:text-lg font-bold bg-gradient-to-r from-amber-500 to-orange-500 text-white rounded-full shadow-lg">
                    Popular
                  </span>
                </div>
              )}
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex-1">
                    ...
                    <p className={`text-xs sm:text-sm sm:text-base sm:text-lg font-medium ${
      ...
      {showPreview && selectedOption && (
        ...
        >
          <div className="flex items-center gap-2 mb-3">
            <Smartphone className="text-gray-200" size={16} />
            <p className="text-sm sm:text-base sm:text-lg font-medium text-gray-300">Notification Preview</p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="p-3 bg-gray-800/50 rounded-lg border border-gray-700/50">
              <div className="flex items-start gap-2 sm:gap-3">
                ...
                <div className="flex-1">
                  <p className="text-sm sm:text-base sm:text-lg font-semibold text-white mb-0.5">
                  ...
                  <p className="text-xs sm:text-sm sm:text-base sm:text-lg text-gray-300 mt-1">
            ...
            {(value === '1hour' || value === '2hours' || value === '1day') && (
              <div className="p-2 bg-blue-500/10 rounded-lg">
                <p className="text-xs sm:text-sm sm:text-base sm:text-lg text-blue-300">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2 sm:gap-3">
        <div className="p-3 bg-gradient-to-r from-green-500/10 to-emerald-500/10 rounded-lg border border-green-500/20">
          <div className="flex items-center gap-2 mb-1">
            ...
            <p className="text-xs sm:text-sm sm:text-base sm:text-lg font-semibold text-green-300">Auto-Adjust</p>
          </div>
          <p className="text-xs sm:text-sm sm:text-base sm:text-lg text-gray-200">
        ...
        <div className="p-3 bg-gradient-to-r from-purple-500/10 to-violet-500/10 rounded-lg border border-purple-500/20">
          <div className="flex items-center gap-2 mb-1">
            ...
            <p className="text-xs sm:text-sm sm:text-base sm:text-lg font-semibold text-purple-300">Multi-Channel</p>
          </div>
          <p className="text-xs sm:text-sm sm:text-base sm:text-lg text-gray-200">
