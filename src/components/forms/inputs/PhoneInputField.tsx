      ...
      <div className="flex gap-2">
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center gap-2 px-3 py-1.5 sm:py-2.5 bg-white/10 border border-white/20 rounded-lg text-white hover:bg-white/15 focus:border-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-400/50 transition-all cursor-pointer h-full"
          >
            <span className="text-lg">{selectedCountry.flag}</span>
            <span className="text-sm sm:text-base sm:text-lg font-medium">{selectedCountry.code}</span>
                ...
                <div className="max-h-64 overflow-y-auto">
                  {COUNTRY_CODES.map(country => (
                    <button
                      ...
                      className={`w-full flex items-center justify-between px-3 sm:px-4 py-1.5 sm:py-2.5 text-left hover:bg-white/10 transition-colors ${
                    ...
                    >
                      <div className="flex items-center gap-2 sm:gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-xs sm:text-sm sm:text-base sm:text-lg opacity-70">{country.code}</div>
                        </div>
                      </div>
        ...
        <div className="flex-1 relative">
          <div className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
            <Phone size={18} className={`${!isValid && phoneNumber ? 'text-red-400' : 'text-gray-200'}`} />
          </div>
          <input
            type="tel"
            name={name}
            ...
            required={required}
            className={`w-full pl-10 pr-4 py-1.5 sm:py-2.5 bg-white/10 border rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-400/50 transition-all ${
              !isValid && phoneNumber 
          ...
          {!isValid && phoneNumber && (
            <motion.p
              ...
              className="absolute -bottom-5 left-0 text-xs sm:text-sm sm:text-base sm:text-lg text-red-400"
            >
