...
export default function IntroWelcomeScreen() {
  ...
  // Optimized scroll to top when step changes - smooth and immediate
  useEffect(() => {
    // Only scroll if we're not already at the top to prevent unnecessary operations
    if (window.scrollY > 0) {
  ...
  useEffect(() => {
    // Immediate scroll to top on mount, only if needed
    if (window.scrollY > 0) {
      window.scrollTo({ top: 0, behavior: 'instant' });
    }
  }, []);
