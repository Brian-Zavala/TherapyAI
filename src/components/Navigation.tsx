...
export default function Navigation() {
  ...
  return (
    <header className="bg-gradient-to-bl from-zinc-800 to-indigo-900 shadow-sm sticky top-0 z-40">
      <div className="w-full px-2 py-3">
        <div className="flex justify-between items-center">
          ...
          <div className="flex-none">
            <button 
              className="p-2 focus:outline-none cursor-pointer rounded-lg hover:bg-indigo-600 transition-colors"
              onClick={toggleMenu}
      ...
      <div 
        className={`fixed top-0 right-0 h-screen bg-gradient-to-bl from-zinc-800 to-indigo-900 z-50 overflow-y-auto shadow-2xl transition-all duration-300 ease-in-out w-72 transform ${
        ...
        <div className="px-4 py-6 flex flex-col space-y-2">
          ...
          {isLoading ? (
            ...
            <>
              <Link 
                href="/auth/login" 
                className={`${linkStyles(pathname === '/auth/login')} py-2 px-3 rounded-lg hover:bg-indigo-800 transition-colors flex items-center`}
                onClick={() => setIsMenuOpen(false)}
              >
                <svg className="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
              ...
              <Link 
                href="/auth/register" 
                className={`${linkStyles(pathname === '/auth/register')} py-2 px-3 rounded-lg hover:bg-indigo-800 transition-colors flex items-center`}
                onClick={() => setIsMenuOpen(false)}
              >
                <svg className="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
