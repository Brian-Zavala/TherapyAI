...
export default function Navigation() {
  ...
  const linkStyles = (isActive) =>
    isActive ? "text-white font-bold" : "text-indigo-100 hover:text-white";

  return (
    <header className="bg-stone-600/85 shadow-sm sticky top-0 z-40 w-full">
      <div className="w-full py-3">
        <div className="flex justify-between items-center w-full px-2">
          ...
          <div className="flex-none pr-6">
            <button
              className="p-2 focus:outline-none cursor-pointer rounded-lg hover:bg-white/35 transition-colors"
              onClick={toggleMenu}
      ...
      <div
        className={`fixed top-0 right-0 h-screen bg-gradient-to-r from-stone-500 to-stone-500 z-50 overflow-y-auto shadow-2xl transition-all duration-300 ease-in-out w-72 transform ${
          isMenuOpen ? "translate-x-0" : "translate-x-full"
        }`}
      >
        <div className="pt-4 px-4 flex justify-end items-center border-b border-stone-800 pb-4">
          <button
            onClick={toggleMenu}
        ...
        <div className="px-4 py-6 flex flex-col space-y-2">
          <Link
            href="/"
            className={`${linkStyles(pathname === "/")} py-2 px-3 rounded-lg hover:bg-black transition-colors flex items-center`}
