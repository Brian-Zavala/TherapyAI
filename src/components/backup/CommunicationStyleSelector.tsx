...
export default function CommunicationStyleSelector({
  ...
  const getColorClasses = (color: string, isSelected: boolean) => {
    const colorMap: Record<string, { bg: string; border: string; icon: string }> = {
      ...
      pink: {
        ...
        icon: isSelected ? 'bg-pink-500/20 text-pink-400' : 'bg-white/10 text-gray-200'
      },
      blue: {
      ...
      teal: {
        ...
        icon: isSelected ? 'bg-teal-500/20 text-teal-400' : 'bg-white/10 text-gray-200'
      }
    };
  ...
  return (
    <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-3 ${className}`}>
