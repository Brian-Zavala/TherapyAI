  ...
  const getColorClasses = (color: string, isSelected: boolean) => {
    const colorMap: Record<string, { bg: string; border: string; icon: string; badge: string; bar: string }> = {
      amber: {
        ...
        icon: isSelected ? 'bg-amber-500/20 text-amber-400' : 'bg-white/10 text-gray-200',
  ...
  return (
    <div className={`space-y-4 ${className}`}>
            ...
            >
              {option.recommended && (
                <div className="absolute -top-2 -right-2">
                  <motion.span 
                    ...
                    className={`inline-flex items-center gap-1 px-2.5 py-1 text-xs sm:text-sm sm:text-base sm:text-lg font-bold bg-gradient-to-r ${colors.badge} text-white rounded-full shadow-lg`}
                  >
                    <TrendingUp size={12} />
                    Best Results
                  </motion.span>
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-start gap-2 sm:gap-3">
                  ...
                  <div className="flex-1">
                    <h4 className={`font-bold text-base sm:text-lg mb-0.5 ${
                      isSelected ? 'text-white' : 'text-gray-200'
                    }`}>
                      {option.label}
                    </h4>
                    <p className={`text-xs sm:text-sm sm:text-base sm:text-lg font-medium mb-1 ${
                ...
                {option.intensity !== 'flexible' && (
                  <div className="flex items-center gap-2">
                    <span className="text-xs sm:text-sm sm:text-base sm:text-lg text-gray-300">Intensity:</span>
                    ...
                    <span className={`text-xs sm:text-sm sm:text-base sm:text-lg capitalize ${
                      isSelected ? 'text-gray-300' : 'text-gray-300'
                    }`}>
                      {option.intensity}
                    </span>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {option.benefits.map((benefit, index) => (
                    <motion.span
                      ...
                      className={`inline-block px-2 py-0.5 text-xs sm:text-sm sm:text-base sm:text-lg rounded-full ${
                        isSelected
      ...
      <div className="p-3 sm:p-4 bg-black/30 rounded-xl border border-white/10">
        <h4 className="text-sm sm:text-base sm:text-lg font-semibold text-white mb-3">Session Impact Over Time</h4>
        <div className="space-y-2">
          {FREQUENCY_OPTIONS.filter(opt => opt.intensity !== 'flexible').map((option) => {
            ...
            return (
              <div key={option.value} className="flex items-center gap-2 sm:gap-3">
                <span className={`text-xs sm:text-sm sm:text-base sm:text-lg w-20 ${
                  value === option.value ? 'text-white font-medium' : 'text-gray-300'
                }`}>
                  {option.label}
                </span>
                ...
                <span className="text-xs sm:text-sm sm:text-base sm:text-lg text-gray-300 w-12 text-right">
                  {progress}%
                </span>
              </div>
            );
        ...
        <p className="text-xs sm:text-sm sm:text-base sm:text-lg text-gray-300 mt-3">
