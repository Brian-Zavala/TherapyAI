  ...
  return (
    <div className={`relative ${className}`}>
            ...
            <AnimatePresence>
              {selectedDayObjects.map(day => (
                <motion.div
                  ...
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-purple-500/20 text-purple-300 rounded-full text-sm sm:text-base sm:text-lg backdrop-blur-sm border border-purple-500/30 hover:bg-purple-500/30 transition-colors cursor-pointer"
                >
                  ...
                  <button
                    ...
                    className="hover:bg-white/10 rounded-full p-0.5 transition-colors cursor-pointer cursor-pointer focus:outline-none focus:ring-2 focus:ring-blue-400/50 focus:ring-2 focus:ring-purple-400/50"
              ...
              {includeTimePreferences && selectedTimeObjects.map(time => (
                <motion.div
                  ...
                  exit={{ scale: 0, opacity: 0 }}
                  transition={{ type: "spring", stiffness: 300, damping: 20 }}
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-amber-500/20 text-amber-300 rounded-full text-sm sm:text-base sm:text-lg backdrop-blur-sm border border-amber-500/30 hover:bg-amber-500/30 transition-colors cursor-pointer"
                >
                  <span>{time.icon}</span>
      ...
      <button
        ...
        className="w-full px-3 sm:px-4 py-1.5 sm:py-2.5 bg-white/10 border border-white/20 rounded-lg text-white focus:border-purple-400 focus:outline-none focus:ring-2 focus:ring-blue-400/50 transition-all cursor-pointer hover:bg-white/15 flex items-center justify-between group"
      >
        <span className={`flex items-center gap-2 ${selectedDayObjects.length === 0 && selectedTimeObjects.length === 0 ? 'text-gray-200' : ''}`}>
          ...
          >
            ...
            {includeTimePreferences && (
              <div className="flex border-b border-white/10 bg-black/30">
                <button
                  ...
                  className={`flex-1 px-3 sm:px-4 py-1.5 sm:py-2.5 text-sm sm:text-base sm:text-lg font-medium transition-all ${
                    activeTab === 'days' 
            ...
            {(!includeTimePreferences || activeTab === 'days') && (
              <div className="flex gap-2 p-3 border-b border-white/10 bg-black/20">
                <button
                  ...
                  className="px-3 py-1 text-xs sm:text-sm sm:text-base sm:text-lg bg-purple-500/20 hover:bg-purple-500/30 text-purple-300 rounded-full transition-colors cursor-pointer"
                >
                  Weekdays
                </button>
                <button
                  ...
                  {DAYS_OF_WEEK.map(day => {
                    ...
                    return (
                      <motion.button
                        ...
                        className={`
                          flex items-center justify-between px-3 sm:px-4 py-1.5 sm:py-2.5 rounded-lg text-sm sm:text-base sm:text-lg text-left transition-all
                          ${isSelected 
                            ? 'bg-purple-500/20 text-purple-300 border border-purple-500/30 shadow-sm' 
                            : isDisabled
                              ? 'bg-gray-800/30 text-gray-400 cursor-not-allowed opacity-50'
                      ...
                      >
                        <div className="flex items-center gap-2 sm:gap-3">
                          ...
                          <span className="text-xs sm:text-sm sm:text-base sm:text-lg opacity-60">({day.abbreviation})</span>
                        </div>
                        {isSelected && (
                          <motion.div
                            initial={{ scale: 0 }}
            ...
            {(selectedDays.length > 0 || selectedTimes.length > 0) && (
              <div className="p-3 border-t border-white/10 flex items-center justify-between bg-black/20">
                <span className="text-sm sm:text-base sm:text-lg text-gray-200">
                ...
                <button
                  ...
                  className="text-sm sm:text-base sm:text-lg text-red-400 hover:text-red-300 transition-colors cursor-pointer"
                >
