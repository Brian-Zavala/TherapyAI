  ...
  const getColorClasses = (color: string, isEnabled: boolean) => {
    const colorMap: Record<string, { bg: string; border: string; icon: string; toggle: string }> = {
      blue: {
        ...
        icon: isEnabled ? 'bg-blue-500/20 text-blue-400' : 'bg-gray-800 text-gray-500',
      ...
      green: {
        ...
        icon: isEnabled ? 'bg-green-500/20 text-green-400' : 'bg-gray-800 text-gray-500',
      ...
      purple: {
        ...
        icon: isEnabled ? 'bg-purple-500/20 text-purple-400' : 'bg-gray-800 text-gray-500',
        toggle: isEnabled ? 'bg-purple-500' : 'bg-gray-700'
      },
      amber: {
        bg: 'bg-gray-800/30',
        border: 'border-gray-700/30',
        icon: 'bg-gray-800/50 text-gray-600',
        toggle: 'bg-gray-800'
      }
    };
    ...
    <div className={`space-y-4 ${className}`}>
      ...
      <div className="flex items-center justify-between p-3 bg-gradient-to-r from-indigo-500/10 to-purple-500/10 rounded-lg border border-indigo-500/20">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400">Choose how you want to receive reminders</p>
          </div>
        </div>
        <button
          type="button"
        ...
        >
          {enabledChannels.size === channels.filter(c => c.id !== 'voice').length ? (
            <>
              <BellOff size={14} />
              <span className="text-xs text-gray-300">Disable All</span>
            </>
          ) : (
            <>
              <Bell size={14} />
              <span className="text-xs text-gray-300">Enable All</span>
            </>
          )}
        </button>
      </div>
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between mb-1">
                    <h4 className={`font-semibold text-sm ${
                      channel.enabled && !isComingSoon ? 'text-white' : 'text-gray-400'
                    }`}>
                      {channel.label}
                    </h4>
                  ...
                  <p className={`text-xs mb-2 ${
                    channel.enabled && !isComingSoon ? 'text-gray-300' : 'text-gray-500'
                  }`}>
                    {channel.description}
                  </p>
        ...
        <div className="space-y-2">
          {enabledChannels.size === 0 ? (
            <div className="flex items-center gap-2 text-gray-500">
              <BellOff size={16} />
              <p className="text-sm">No notifications enabled</p>
            </div>
          ) : (
      ...
      <AnimatePresence>
        {showTestNotification && enabledChannels.size > 0 && (
          ...
          >
            <p className="text-sm font-semibold text-white mb-3">Test Notification Sent!</p>
            ...
            <div className="space-y-2">
              {Array.from(enabledChannels).map(channelId => {
                ...
                return messages[channelId] ? (
                  <p key={channelId} className="text-xs text-blue-300">
                    {messages[channelId]}
                  </p>
                ) : null;
              })}
