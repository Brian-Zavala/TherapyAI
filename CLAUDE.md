...
- Add cursor-pointer where needed
...
### React Conditional Rendering Issues (Jan 2025)

**Problem**: Component logic passes but JSX never renders to DOM
**Root Cause**: String matching bugs in conditional rendering chains

**Debug Strategy**:
1. **Add DOM inspection**: `document.querySelector()` to verify element exists
2. **Add pipeline debugging**: Log every conditional check and return statement
3. **Check string matching**: Be specific with `.startsWith()` and similar methods

**Common Pitfall**: Overlapping string matches in field names:
```javascript
// BUG: "familyMemberCount".startsWith("familyMember") = true
if (field.name.startsWith("familyMember") && hasFamily === "yes") {
  // This catches familyMemberCount when it shouldn't
}

// FIX: Be specific about exclusions
if (field.name.startsWith("familyMember") && 
    field.name !== "familyMemberCount" && 
    hasFamily === "yes") {
  // Now familyMemberCount bypasses this logic
}
```

### State Management with Persistent Storage

**Issue**: localStorage saves triggering excessive re-renders
**Solution**: Debounce localStorage operations (100ms minimum)
```javascript
useEffect(() => {
...
**Debug Tools**:
- **State pipeline**: Log state updates from hook through component render
- **DOM verification**: Check if JSX actually reaches DOM vs just passes logic
- **React keys**: Use dynamic keys to force re-reconciliation when debugging
...
- **Conditional Rendering**: Be specific with string matching to avoid overlapping conditions
